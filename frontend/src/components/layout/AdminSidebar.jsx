...
const AdminSidebar = () => {
  ...
  // ============================================
  // STUDENT / CLEANER sidebar (keep as-is)
  // ============================================
  if (user?.role === 'student' || user?.role === 'clean') {
    ...
    return (
      <aside className="w-72 bg-white flex flex-col shadow-2xl z-20 hidden md:flex border-r border-gray-100">
        <div className="p-5">
          <div className="bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-900 rounded-[2.5rem] p-8 text-white shadow-xl relative overflow-hidden flex flex-col items-center text-center">
            ...
            <div className="w-20 h-20 rounded-full bg-blue-950 border-[3px] border-amber-400 flex items-center justify-center font-bold text-2xl shadow-[0_0_15px_rgba(251,191,36,0.6)] uppercase tracking-wide">
              {initials}
            </div>
            <h3 className="mt-4 font-bold text-xl leading-tight tracking-wide drop-shadow-sm truncate max-w-full">
              {user?.fullName || user?.name || 'User'}
            </h3>
            <p className="text-xs text-blue-200/80 mt-1 truncate max-w-full font-medium">
              {user?.email || 'user@gmail.com'}
            </p>
            <button
              onClick={toggleDuty}
              className="mt-4 px-4 py-1.5 rounded-full text-[11px] font-bold tracking-wider text-blue-100 bg-white/10 hover:bg-white/20 border border-white/20 flex items-center gap-1.5 transition-all duration-200 active:scale-95 shadow-sm"
            >
  ...
  // ============================================
  // ADMIN / STAFF sidebar — WHITE design matching screenshot
  // ============================================
  return (
    <aside className="w-56 bg-white flex flex-col z-20 hidden md:flex border-r border-gray-200 flex-shrink-0">
      {/* Logo */}
      <div className="h-16 flex items-center px-5 border-b border-gray-100 flex-shrink-0">
        <div className="w-8 h-8 bg-indigo-600 rounded-lg flex items-center justify-center font-bold text-sm text-white flex-shrink-0">
          SC
        </div>
        <h1 className="ml-3 text-base font-bold text-gray-800 tracking-tight">Smart Campus</h1>
      </div>
      ...
      <div className="p-3 border-t border-gray-100 flex-shrink-0">
        <button
          ...
          className="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm font-medium text-gray-500 hover:text-red-600 hover:bg-red-50 transition-all duration-150"
        >
