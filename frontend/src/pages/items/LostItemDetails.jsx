        ...
        <div className="grid grid-cols-1 md:grid-cols-3">
          ...
          <div className="bg-gray-50 flex items-center justify-center p-8 border-b md:border-b-0 md:border-r border-gray-200">
            {item.image || item.imageUrl ? (
              <img src={getImageUrl(item)} alt={item.title} className="max-w-full h-auto rounded-xl shadow-md object-cover max-h-96" />
            ) : (
          ...
          <div className="md:col-span-2 p-8">
            <div className="flex justify-between items-start mb-8">
              ...
              <div className="text-right">
                ...
                <p className="text-xl text-gray-800 font-bold">{new Date(item.dateLost || item.createdAt).toLocaleDateString()}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-10">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-900 font-bold">{item.location}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-900 font-bold">{item.createdBy?.fullName || item.createdBy?.name || item.createdBy?.email || ''}</p>
                  <p className="text-xs text-gray-500 font-medium">{item.createdBy?.email || ''}</p>
                </div>
              </div>
            </div>
            ...
            <div className="pt-8 border-t border-gray-100 flex flex-wrap gap-4">
              <button 
                ...
                className="px-8 py-3 bg-blue-100 text-blue-700 rounded-xl font-bold hover:bg-blue-200 transition-all flex items-center gap-2 shadow-sm"
              >
        ...
        {item.linkedFoundItem ? (
          <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 mb-6">
            <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
              ...
              Identity Matching & Claims
            </h3>
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex justify-between items-center">
              <div>
                <p className="font-semibold text-blue-900">Linked to Found Item Tracker:</p>
                <p className="text-sm text-blue-700">Database ID: {item.linkedFoundItem._id || item.linkedFoundItem}</p>
              </div>
              <Link to={`/admin/found-items/${item.linkedFoundItem._id || item.linkedFoundItem}`} className="bg-white text-blue-600 font-medium px-4 py-2 rounded-md shadow-sm border border-blue-200 hover:bg-blue-50">View Found Record</Link>
            </div>
          </div>
        ) : null}
                ...
                <div className="flex h-40">
                  <div className="w-1/3 bg-gray-100 relative overflow-hidden">
                    {match.foundItemId.image || match.foundItemId.imageUrl ? (
                      <img src={getImageUrl(match.foundItemId)} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" alt="" />
                    ) : (
