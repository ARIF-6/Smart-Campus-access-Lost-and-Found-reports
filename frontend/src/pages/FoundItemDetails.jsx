...
const FoundItemDetails = () => {
  ...
  const fetchItemData = async () => {
    try {
      ...
      const lostData = await getLostItems({ status: 'lost' });
      setLostItemsList(lostData);
    } catch (err) {
  ...
  const handleLinkLostItem = async () => {
    ...
    try {
      ...
      fetchItemData();
      alert('Items Successfully Linked.');
    } catch (err) {
      alert('Error forming Link.');
    }
  }
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3">
          ...
          <div className="bg-gray-100 flex items-center justify-center p-8 border-b md:border-b-0 md:border-r border-gray-200">
            {item.image ? (
              <img src={`http://localhost:5000${item.image}`} alt={item.title} className="max-w-full h-auto rounded-xl shadow-sm object-cover" />
            ) : (
          ...
          <div className="md:col-span-2 p-8">
            ...
            <div className="grid grid-cols-2 gap-6 mb-8">
              ...
              <div>
                ...
                <p className="text-gray-800 font-medium">{item.locationFound}</p>
              </div>
              <div>
                 <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-1">Reported By</h3>
                 <p className="text-gray-800 font-medium">{item.reportedBy?.name || 'Unknown User'}</p>
